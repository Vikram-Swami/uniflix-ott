    ...
    <nav className={`fixed left-1/2 -translate-x-1/2 w-full md:px-[23px] 2xl:px-[51px]! md:rounded-b-xl z-5000 transition-all duration-300 ease-in-out`}>
      <div className={`flex relative items-center justify-between h-[50px] md:h-[60px] 2xl:h-[70px] px-3 xs:px-[21px] md:rounded-b-xl transition-all duration-300 ease-in-out ${isScrolled || movieDetailsPopupScroll ? "nav_bg_leaner2" : "nav_bg_leaner"
        ...
        <div ref={menuRef} className="flex items-center xs:gap-17 gap-4">
          ...
          {isUser && <button onClick={() => setIsOpenDropdown(!isOpenDropdown)} className="md:hidden cursor-pointer"><TextAlignJustify className="max-xs:w-5 max-xs:h-5" /></button>}
          {isOpenDropdown && isUser && <div div className="flex max-md:flex-col md:items-center text-sm 2xl:text-[16px] font-semibold gap-1 md:gap-[5px] max-md:absolute top-[50px] mt-0.5 max-md:[background:linear-gradient(to_bottom,#191e25cc,#191e25cc)] max-md:w-[150px] max-md:px-3 py-2 max-md:rounded-md max-md:left-3">
            <NavLink onClick={() => {
              if (window.innerWidth < 768) {
                setIsOpenDropdown(false)
              }
        ...
        <div className="flex items-center gap-3">
          ...
          <div ref={menuRef2} className="relative w-[28px] h-[28px] lg:w-[33px] lg:h-[33px] max-xs:w-6 max-xs:h-6">
            ...
            {(userDropdown && isUser) && <div className="absolute top-10 xs:top-11 md:top-13 md:-right-[21px] nav_bg_leaner2 w-40 xs:w-50 right-0 px-1 md:px-2 py-2 rounded-md flex flex-col">
